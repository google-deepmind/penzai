...
from penzai.toolshed import check_layers_by_tracing
...
class LoraTest(absltest.TestCase):
  ...
  def test_build_lora(self):
    ...
    loraified_mlp = lora.loraify_linears_in_selection(pz.select(mlp), rank=2)
    (
    ...
    for param in (
      ...
      self.assertIn("lowrank", param.value_structure.named_shape)
      self.assertEqual(param.value_structure.named_shape["lowrank"], 2)
    check_layers_by_tracing.check_layer(
        loraified_mlp, pz.nx.zeros({"batch": 3, "features": 8})
    )
  ...
  def test_can_finetune_with_lora(self):
    ...
    frozen_mlp = (
        ...
        .apply(lambda param: pz.nn.FrozenParameter(param.value, param.name))
    )
    # LoRA-ify it and learn the LoRA parameters.
    lora_mlp = lora.loraify_linears_in_selection(pz.select(frozen_mlp), rank=4)
    init_lora_mlp = pz.nn.initialize_parameters(lora_mlp, jax.random.key(20))
