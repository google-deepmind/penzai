...
def temperature_sample_pyloop(
  ...
  Args:
    ...
    prompt: A named array of prompt tokens. Must have a "seq" axis along which
      ...
      the beginning of sequence token. Must end with a non-padding token; if the
      prompt includes padding the padding should be placed at the beginning.
  ...
  next_log_probs = model(prompt)[{"seq": -1}]
  sampled_count = 0
