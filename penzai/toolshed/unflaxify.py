...
class InterceptedFlaxScopeData(pz.Struct):
  ...
  Attributes:
    ...
    variables: The collection of other variables used directly by this module
      (not a submodule).
    immutable_variables: The collection of immutable variable values used
      directly by this module (not a submodule).
    rng_names: Names for the RNGs used by this module method, which will be
      converted to Flax random number states. Note that the random numbers
      generated by Penzai will NOT exactly match the random numbers generated by
      Flax, because Flax has custom logic for splitting and seeding RNGs that is
      not easy to directly reproduce in Penzai.
  """

  parameters: dict[str, pz.Parameter[Any]]
  variables: dict[str, dict[str, pz.StateVariable[Any]]]
  immutable_variables: dict[str, dict[str, Any]]
  rng_names: frozenset[str] = dataclasses.field(metadata={"pytree_node": False})


@dataclasses.dataclass(frozen=True)
class FlaxVarLabel(pz.AutoOrderedAcrossTypes):
...
@pz.pytree_dataclass
class InterceptedFlaxModuleMethod(pz.nn.Layer):
  ...
  def __call__(
      self,
      args_and_kwargs: ArgsAndKwargs,
      random_streams: dict[str, pz.RandomStream] | None = None,
      **side_inputs,
  ) -> Any:
    ...
    Args:
      args_and_kwargs: The positional and keyword arguments passed to the Flax
        method.
      random_streams: Random number generators. Should include a key for each
        RNG used by the Flax module method. Note that RNG states will NOT
        necessarily be the same between Flax and Penzai.
      **side_inputs: Other Penzai side inputs for the model, which will be
        forwarded to other penzai layers. (Only used if penzai layers that use
        side inputs have been inserted.)
    ...
    flax_variables = {
        ...
        **{
            col: {k: v.value for k, v in col_vars.items()}
            for col, col_vars in scope_data.variables.items()
        },
        **scope_data.immutable_variables,
    }
    flax_rngs = {}
    for k in scope_data.rng_names:
      if k not in random_streams:
        raise TypeError(
            "InterceptedFlaxModuleMethod requires a RandomStream named"
            f" {repr(k)}, but it was not found in the `random_streams` side"
            " input."
        )
      flax_rngs[k] = random_streams[k].next_key()
    ...
    def redirecting_interceptor(next_fun, args, kwargs, context):
      ...
      if the_bound_module is None:
        ...
        return subcall(
            ArgsAndKwargs(args=args, kwargs=kwargs),
    ...
    for col, new_col_vars in new_variables.items():
      for k, v in new_col_vars.items():
        scope_data.variables[col][k].set_value(v)
    return output
...
def unflaxify_apply(
  ...
  # Store the values for each variable.
  # This is a bit fragile. We want to call the Flax model under eval_shape so
  # that we don't need to run any compute. But it's not necessarily possible to
  # identify what the variables are until we actually identify the linen module
  # that owns each variable, since dictionaries can be nested arbitrarily.
  # This means that, when we identify the variable, we are running under
  # eval_shape.
  # To avoid creating a Penzai variable inside the eval_shape, we instead save
  # the value that that variable should have into this dictionary. If that
  # value depended on computations inside the eval_shape, this would leak a
  # tracer. But the initial value of each variable never depends on any
  # computations inside the eval_shape, so it's OK to save it here.
  # Later, outside the eval_shape, we can create the top-level pz.Variable
  # instances and substitute them in.
  pz_parameter_values = collections.defaultdict(dict)
  pz_state_var_values = collections.defaultdict(dict)
  ...
  def reifying_interceptor(next_fun, args, kwargs, context):
    ...
    parameter_vars, variables, immutable_variables = (  # pylint: disable=unbalanced-tuple-unpacking
    ...
    if parameter_vars:
      for k, v in parameter_vars["params"].items():
        if context.module.scope.name_reserved(k, col="params"):
          param_label = name_prefix + k
          if param_label not in pz_parameter_values:
            pz_parameter_values[param_label] = v
          converted_parameters[k] = pz.ParameterSlot(param_label)
    ...
    for col, col_vars in variables.items():
      ...
      for k, v in col_vars.items():
        if context.module.scope.name_reserved(k, col=col):
          var_label = FlaxVarLabel(collection=col, name=name_prefix + k)
          if var_label not in pz_state_var_values:
            pz_state_var_values[var_label] = v
          converted_variables[col][k] = pz.StateVariableSlot(var_label)
  ...
  all_vars = [
      pz.Parameter(value=v, label=l) for l, v in pz_parameter_values.items()
  ] + [
      pz.StateVariable(value=v, label=l) for l, v in pz_state_var_values.items()
  ]
  return pz.bind_variables(root_intercept, all_vars)
